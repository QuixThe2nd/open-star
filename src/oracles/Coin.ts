...
const reputationChange = (peer: `0x${string}`, reputation: number): void => {
  ...
  if (reputation > 0) {
    ...
    openStar.mint({ to: peer, amount: (state.value.balances[peer] !== undefined ? BigInt(Math.floor(Number(state.value.balances[peer])*blockYield)) : parseEther(100)).toHex().value });
