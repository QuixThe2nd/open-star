...
export function sign(hash: `0x${string}`, privateKey: `0x${string}`): `0x${string}` {
  const signature = secp256k1.sign(hash.slice(2), privateKey.slice(2), { lowS: true, extraEntropy: false })
...
export function verify(address: `0x${string}`, message: string, signature: Hex): boolean {
  ...
  const publicKey = `0x${secp256k1.Signature.fromCompact(signature.value.substring(2, 130)).addRecoveryBit(toRecoveryBit(new Hex(`0x${signature.value.slice(130)}`).number)).recoverPublicKey(keccak256(toPrefixedMessage(message)).substring(2)).toHex(false)}`
