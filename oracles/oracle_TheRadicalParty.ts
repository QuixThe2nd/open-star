import { type NonEmptyArray, type ORC20State, OpenStar, OpenStarRC20, type Oracle, StateManager, mode, parseEther } from '../src'
...
const methodDescriptions: {
	[K in keyof typeof methods]: Parameters<(typeof methods)[keyof typeof methods]>[0]
} = {
...
function reputationChange(peer: `0x${string}`, reputation: number) {
	...
	if (reputation > 0) {
		...
		openStar.mint({
			to: peer,
			amount: (balance !== undefined ? BigInt(Math.floor(Number(balance) * epochYield)) : parseEther(1)).toHex().value
		})
	} else if (reputation < 0 && balance !== undefined) {
		console.log('[COIN] Slashing', peer.slice(0, 8) + '...')
		openStar.burn({
			to: peer,
			amount: ((BigInt(balance) * 9n) / 10n).toHex().value
		})
	}
}
